#ifndef SEND_MORE_MONEY
#define SEND_MORE_MONEY
#define TEN 10
#define SIZE 8
#include <stdio.h>
...
static int *_E = _Digits + 1;
static int *_N = _Digits + 2;
static int *_D = _Digits + 3;
static int *_M = _Digits + 4;
static int *_O = _Digits + 5;
static int *_R = _Digits + 6;
static int *_Y = _Digits + 7;

static void _Doit(int *arr, int num) {
	if (num == 1) {
		if (*_S != 0 && *_M != 0) {
			int a = *_S * 1000 + *_E * 100 + *_N * 10 + *_D;
