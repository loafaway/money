#ifndef SPEND_LESS_MONEY
#define SPEND_LESS_MONEY
#define TEN 10
#define SIZE 8
#include <stdio.h>
...
static int *_P = _Digits + 1;
static int *_E = _Digits + 2;
static int *_N = _Digits + 3;
static int *_D = _Digits + 4;
static int *_L = _Digits + 5;
static int *_M = _Digits + 6;
static int *_O = _Digits + 7;
static int *_Y = _Digits + 8;

static void _Doit(int *arr, int num) {
	if (num == 1) {
		if (*_S != 0 && *_L != 0 && *_M != 0) {
			int a = *_S * 10000 + *_P * 1000 + *_E * 100 + *_N * 10 + *_D;
			int b = *_L * 1000 + *_E * 100 + *_S * 10 + *_S;
